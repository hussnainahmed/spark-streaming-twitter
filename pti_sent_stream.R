...
raw = fetch(dbSendQuery(mydb, 'SELECT text,STR_TO_DATE(created_at,\'%W %b %d %T +0000 %Y\') as dt from tweet_text where STR_TO_DATE(created_at,\'%W %b %d %T +0000 %Y\') > (NOW() - INTERVAL 5 MINUTE)'))
dbDisconnect(mydb)
