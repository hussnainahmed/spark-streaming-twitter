...
def functionToCreateContext():
      ...
      def process(rdd):
          try:
              ...
              pti_text = sqlContext1.sql("select text,created_at, entities.hashtags from ts where text IS NOT NULL and lang=\'en\'")
  	      ...
  	      df['created_at']= df['created_at'].map(lambda y: (datetime.strptime(y,'%a %b %d %H:%M:%S +0000 %Y')+ timedelta(hours=5)))
              ...
              #conn.close()
              #df.sqlContext1.createDataFrame(poped).show()
	      print(df)
...
if __name__ == "__main__":
    checkpointDirectory = '/user/flume/tweet-stream/hashtrack_cp/'
